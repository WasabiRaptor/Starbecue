function init()
	...
	if storage.state == nil then
		storage.state = config.getParameter("defaultState") == "open"
	end
	animator.setAnimationState("doorState", storage.locked and "locked" or storage.state and "open" or "closed")
...
function onInputNodeChange()
	...
	if storage.locked then return end
...
function openDoor(direction)
	if not storage.state and not storage.locked then
		storage.state = true
