...
function widgets.slider:draw()
    ...
    if #self.handles > 1 then
        c:drawLine({8, m}, {self.size[1] - 8, m}, {0, 0, 0}, 4)
        c:drawLine({math.floor(f * (self.handles[1].value - s)), m}, {math.floor(f * (self.handles[#self.handles].value - s)), m}, {50, 50, 50}, 4)
    else
        c:drawLine({8, m}, {self.size[1] - 8, m}, {50, 50, 50}, 4)
    end

    for i, n in ipairs(self.notches) do
        local color = {50, 50, 50}
