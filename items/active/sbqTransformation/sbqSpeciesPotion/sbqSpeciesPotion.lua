...
function update(dt, fireMode, shiftHeld)
	if fireMode == "primary" and not activeItem.callOtherHandScript("isDartGun") then
		...
		player.giveItem({ name = "sbqMysteriousPotion", parameters = {
			args = {{species = sbq.species()},config.getParameter("duration") or sbq.config.defaultTFDuration},
		}})
...
function transformationItemArgs(useType)
	...
	return { message = "sbqDoTransformation", itemName = "sbqMysteriousPotion", args = {{species= sbq.species()}, config.getParameter("duration") or sbq.config.defaultTFDuration}, consume = not player.isAdmin()}
end
