function sbq.eyeTracking()
	...
	local deadZone = animator.partProperty("eyes", "eyeDeadZone") or sbq.voreConfig.eyeTrackingDeadzone or {1,1}
	local farDist = animator.partProperty("eyes", "eyeFarDist") or sbq.voreConfig.eyeTrackingFarDist or 10
	if (absDist[1] > (deadZone[1])) then
		if ((targetAngle > 292) or (targetAngle < 68)) then
			animator.setGlobalTag("eyesX", (absDist[1] > farDist) and "2" or "1")
