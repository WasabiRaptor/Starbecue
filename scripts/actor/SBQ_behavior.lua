...
function entityInSight(args, ...)
	...
	if res then
		return not world.entity(args.entity):statPositive("sbqIsPrey")
	end
	return res
end
...
function sayToEntity(args, ...)
	if args.dialogType == "dialog.lostTarget" and args.entity and world.entityExists(args.entity) and world.entity(args.entity):statPositive("sbqIsPrey") then
		return true
	end
