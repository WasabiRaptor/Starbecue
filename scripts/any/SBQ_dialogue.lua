...
function dialogueProcessor.processDialogueResults(i)
	...
	local targetName = results.target and world.entityExists(results.target) and (sbq.entityName(results.target) .. "^reset;")
...
function dialogueProcessor.sendPlayerDialogueBox(actions)
	if not sbq.settings.read.interactDialogue then return end
...
function dialogueProcessor.speakDialogue(callback)
	...
	if status.statPositive("sbqIsPrey") and sbq.loungingIn() then
		world.sendEntityMessage(sbq.loungingIn(), "sbqPredHudPreyDialogue", entity.id(),
