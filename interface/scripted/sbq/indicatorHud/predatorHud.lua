...
require("/interface/scripted/sbq/NumberBox.lua")
			...
			if ((not ((i == "0") or (i == 0))) or sbq.sbqCurrentData.species == "sbqOccupantHolder") and (occupant ~= nil) and (type(id) == "number") and (world.entityExists( id )) then
				...
				if type(sbq.occupantList[id]) ~= "table" then
					sbq.occupantList[id] = { layout = occupantsArea:addChild({ type = "layout", mode = "manual", position = {0,y}, size = {96,24}, children = {
						...
						{ type = "image", noAutoCrop = true, position = {0,0}, file = "portraitCover.png"  },
...
function sbq.predUIeffectsPanel(location)
	...
	if (locationData.selectEffect or locationData.TF or locationData.Eggify) then
		...
		noneButton.icon = ((locationData.none or {}).icon or "/interface/scripted/sbq/settings/noEffect.png")
		healButton.icon = ((locationData.heal or {}).icon or "/interface/scripted/sbq/settings/heal.png")
		softDigestButton.icon = ((locationData.softDigest or {}).icon or "/interface/scripted/sbq/settings/softDigest.png")
		digestButton.icon = ((locationData.digest or {}).icon or "/interface/scripted/sbq/settings/digest.png")
		...
		for i, extraEffect in ipairs(locationData.passiveToggles or {}) do
			...
			if toggleData then
				...
				local button = extraEffectTogglesLayout:addChild({
					type = "sbqCheckBox", id = location..extraEffect, checked = sbq.predatorSettings[location..extraEffect],
					...
					icon = ((locationData[extraEffect] or {}).icon or "/interface/scripted/sbq/settings/transform.png")
				})
